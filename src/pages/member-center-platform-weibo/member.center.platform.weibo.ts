...
@Component({
  selector: 'Page-MemberCenterPlatformWeibo',
  templateUrl: 'member.center.platform.weibo.html',
  providers: [Transfer, TransferObject, File]
})
export class MemberCenterPlatformWeiboPage {
  private _imgurl:any = 0;
  private _filename:any = 0;

  private wei_bo:any;
  private weibonum:any;
  private webboIntro:any;
  ...
  constructor(public platform: Platform,
    ...
    if(this.gs.get('userInfo')){
      this.wei_bo = this.gs.get('userInfo')['web_chat'];
      this.weibonum = this.gs.get('userInfo')['webxinnum'];
      this.webboIntro = this.gs.get('userInfo')['webchatIntro'];
      this._imgurl = this.gs.get('userInfo')['weibopic'];
    }
  ...
  submitWeibo(){
    let objs = {
      ...
      'wei_bo':this.wei_bo,
      'weibonum':this.weibonum,
      'webboIntro':this.webboIntro,
      'weibopic':this._filename
    };

    this.homeService.saveWeiboService(objs).then((res) => {
      let _code = res['code'];
      if(_code == '0'){
